...
client = ollama.Client(
    ...
    headers={'Authorization': f'Bearer 542c8bd57ad140b6a2665666b9f52d33.DrDEdEQm2eLI4Z4-ysYEfnGm'} #69f04576c06e4cffbcc077ec6c148f4f.351wFgx1FYNsQRTUjIFN4MNc'} 
)
...
def split_ollama(text):
    ...
    try:
        ...
        naive = [s.strip() for s in naive if s.strip()]  #was re.?
        #print(naive)
        return naive
...
def extract_json(corrupted_json):
    """
    Fixes corrupted JSON using json_repair package
    """
    try:
...
def classify_category(pos_seq, dep_seq):
    ...
    # Find subjects, objects, and verbs
    ...
    # If we have the essential components
    if not subject_indices or not object_indices:
        return "Unknown"

    # Use the first occurrence of each for basic word order
